...
import com.tydic.core.util.function.TriConsumer;
...
public final class Loop {
  ...
  /**
   * Performs the given action for each entry and its index in this map
   * until all entries have been processed or the action throws an exception.
   *
   * @param map The <code>java.util.Map</code> instance
   * @param action The action to be performed for each element
   * @see Loop#forEach(Map, BiConsumer)
   */
  public static <K, V> void forEach(Map<? extends K, ? extends V> map, TriConsumer<Integer, ? super K, ? super V> action) {
    Objects.requireNonNull(map);
    Objects.requireNonNull(action);
    int index = 0;
